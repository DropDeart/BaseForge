...
- [x] `baseforge update <servis>` CLI komutu — `new`'in aksine, diskteki `<servis>/spec.yaml` (ve varsa `identity/auth.yaml`) yüklenmiş olarak Designer'ı açar; var olan servise entity eklemek/düzenlemek için YAML'a elle dokunmaya gerek kalmaz.
  ~~**Bilinen kısıt:** identity servisinin adı `auth.yaml`'da "identity" dışında bir şeye değiştirilmişse `update` onu bulamıyor (klasör adı hardcoded).~~ — 2026-07-17'de çözüldü (`RunUpdate`, `DesignerEndpoints.ResolveIdentityFolderName`'in kullandığı aynı `ServiceRegistry` kaydından gerçek identity klasör adını çözüyor; hedef identity ise `auth.yaml` varlığı kontrol ediliyor, `spec.yaml` değil).
- [x] RabbitMQ / asenkron event iletişimi — `IIntegrationEvent`/`IEventBus` (Core/Infrastructure, MediatR ile yerel dağıtım), `EnableRabbitMq` (API), CodeGen `publishes`/`subscribes` (entity/servis bazlı, kardeş-spec zengin çözümleme). Detay: docs/ARCH.md §5.2. `via: event` kalıcı olarak no-op — read-model senkronizasyonu için ayrı bir gelecek özellik olarak rezerve.
  **Bilinen kısıtlar:** `publishes`/`subscribes`'ın kendisi hâlâ CLI/YAML-only (Designer'da form karşılığı yok). ~~DLQ yok~~, ~~kanal havuzu yok~~, ~~outbox ince ayarları için Designer formu yok~~ — 2026-07-16'da çözüldü (`{queue}.dead` dead-letter kuyruğu, `RabbitMqConnectionManager` rent/return havuzu, `ServiceSpec.RabbitMqTuning` Designer formu — bkz. docs/ARCH.md §5.2). Otomatik gecikmeli retry (N kere yeniden dene, sonra DLQ) hâlâ yok — bilinçli kapsam dışı.
- [x] Transactional Outbox Pattern — DB commit ile RabbitMQ publish arasındaki dual-write riskini (publish başarısız olursa event kaybı) çözer. `IEventBus` artık broker'a doğrudan yazmıyor; `OutboxEventBus` olayı aynı `SaveChangesAsync` transaction'ında bir `OutboxMessage` satırı olarak yazıyor, `OutboxPublisherHostedService` (`FOR UPDATE SKIP LOCKED` ile çoklu-instance güvenli polling relay) bunu ayrı olarak gerçek broker'a gönderiyor. Detay: docs/ARCH.md §5.2.
  **Bilinen kısıtlar:** At-least-once teslim (exactly-once değil). `IUnitOfWork`'ün explicit `BeginTransactionAsync/CommitAsync/RollbackAsync`'i hâlâ hiç kullanılmıyor (ayrı bir sonraki adım — MediatR `TransactionBehavior`). ~~Cleanup/retention job yok~~, ~~sınırsız retry~~, ~~tüketici idempotency'si yok~~ — 2026-07-16'da çözüldü (`OutboxMaxRetries`/`OutboxRetention`, Inbox pattern — bkz. docs/ARCH.md §5.2). Kalan ince kısıt: handler'ın kendi DB etkileri ile Inbox işaretlemesi tek transaction'da değil (mark-after/ack arasında nadir bir duplicate riski kalır).
- [x] Merkezi Loglama (Serilog + Grafana Loki) + Correlation ID — her mikroservis kendi konsol çıktısına hapsolmuş durumdaydı, bir isteği HTTP → gRPC → RabbitMQ event zinciri boyunca takip etmenin yolu yoktu. `ICorrelationIdAccessor` (AsyncLocal, Core/Infrastructure) HTTP middleware, gRPC client/server interceptor'ları ve RabbitMQ outbox/consumer arasında paylaşılıyor; `AddBaseForgeLogging` (API, Host seviyesi) her zaman konsola, `Serilog:LokiUrl` doluysa Grafana Loki'ye de yapılandırılmış log yazıyor. Her zaman açık — RabbitMQ/JWT gibi `spec.yaml` opt-in toggle'ı değil. Detay: docs/ARCH.md §5.6.
  **Bilinen kısıtlar:** Grafana'da hazır dashboard yok (LogQL/Explore ile sorgu), log retention/rotation Loki varsayılanında, Loki'ye bağlanılamazsa sessizce sadece konsola düşülür (ayrı uyarı yok), gRPC client interceptor'ı yalnızca unary çağrıları destekler.
- [x] Health Check + Identity dashboard'unda canlı servis durumu — her üretilen serviste her zaman açık `/health` (BaseForge.API `AddHealthChecks` + `PostgresHealthCheck`, `UseBaseForge` artık `WebApplication` alıyor), docker-compose `healthcheck:` (curl, Dockerfile'a eklendi), Identity'nin `GET /api/services/status` ile diğer servisleri `host.docker.internal` üzerinden yoklaması (kod tabanındaki ilk server-to-server `HttpClient`), dashboard'da 10s'de bir yenilenen canlı rozet. Detay: docs/ARCH.md §5.7.
  **Bilinen kısıtlar:** Geçmişe dönük uptime kaydı yok (sadece anlık), `/health`/`/api/services/status` kimlik doğrulamasız (statik `services.json` ile aynı trust seviyesi), otomatik alarm/bildirim yok.
